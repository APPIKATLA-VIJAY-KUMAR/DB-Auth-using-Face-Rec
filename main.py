...
def verify():
    ...
    if ref_firstName == "" or ref_secondName == "" or ref_email == "" or ref_phone_num == "" or ref_pass == "":
        ...
        if len(ref_admin_tup) >= 1:
            ...
            conn.commit()
            ref_secondary_table_insert = "INSERT INTO SECONDARY_ADMINS VALUES('"+str(ref_firstName)+"','" + str(ref_secondName)+"','" + str(ref_email) +"','"+ str(x) + "','"+ str(ref_pass) +"'," + str(ref_phone_num)+")"
...
def adminPriv():
    ...
    submit_button = Button(privilages, command=verify).grid(row=6, column=2)
...
def addStudents():
    ...
    stu_submit_button = Button(add_stu_window, command=stuVerify).grid(row=7, column=2)
